/**
 * GOD Tactical — Consumable Item Sheet (ItemSheetV2)
 */
...
export class GODConsumableSheet extends HandlebarsApplicationMixin(
  foundry.applications.sheets.ItemSheetV2
) {
  static DEFAULT_OPTIONS = {
    classes: ["god-tactical", "item", "consumable"],
  ...
  static PARTS = {
    sheet: {
      template: "systems/god-tactical/templates/item/consumable-sheet.hbs",
      scrollable: [".ws-body"],
    },
  };
  ...
  async _prepareContext(options) {
    ...
    // Header field (see items.mjs's domain doc comment) — single-select, same field/
    // label shared by Weapon/Armor.
  ...
  _onRender(context, options) {
    ...
    // "-1" next to the Запас dice count — a gameplay action (restocked ammo, patched a
    // shield), not a sheet-edit-only control, so it's always live regardless of isEditing.
    this.element.querySelector(".ws-stock-decrement")?.addEventListener("click", this.#onDecrementStock.bind(this));
  ...
  /** Shrink the stock-dice chain by one — the player restocked ammo or patched a shield.
   *  Floored at 1 (the chain never has zero dice — see items.mjs's stockDice doc
   *  comment); the button itself is already disabled at 1 (see consumable-sheet.hbs). */
  async #onDecrementStock(event) {
    event.preventDefault();
    const current = this.document.system.stockDice;
    if (current <= 1) return;
    await this.document.update({ "system.stockDice": current - 1 });
  }
