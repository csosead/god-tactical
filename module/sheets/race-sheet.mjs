/**
 * GOD Tactical — Race Item Sheet (ItemSheetV2)
 */
...
export class GODRaceSheet extends HandlebarsApplicationMixin(
  foundry.applications.sheets.ItemSheetV2
) {
  static DEFAULT_OPTIONS = {
    classes: ["god-tactical", "item", "race"],
  ...
  static PARTS = {
    sheet: {
      template: "systems/god-tactical/templates/item/race-sheet.hbs",
      scrollable: [".ws-body"],
    },
  };
  ...
  async _prepareContext(options) {
    ...
    // One row per characteristic — a plain signed number the GM types directly, not a
    // repeatable "+1 per click" list (there's only 4 of them, see items.mjs's
    // RaceDataModel.charBonuses doc comment for why).
    context.charBonusRows = Object.values(GOD.SKILL_MAP).map((cat) => {
      const value = item.system.charBonuses?.[cat.charKey] ?? 0;
      return { key: cat.charKey, name: cat.name, value, formatted: value > 0 ? `+${value}` : String(value) };
    });
